...
flags.DEFINE_integer("input_dims_audio", None, "")
flags.DEFINE_integer("input_dims_text", None, "")
...
def main():
	...
	trainQueries, trainOwnHistory, trainOtherHistory, trainOwnHistoryMask, trainOtherHistoryMask, trainLabels, text_trainQueries, audio_trainQueries, \
			valQueries, valOwnHistory, valOtherHistory, valOwnHistoryMask, valOtherHistoryMask, valLabels, text_valQueries, audio_valQueries, \
	        testQueries, testOwnHistory, testOtherHistory, testOwnHistoryMask, testOtherHistoryMask, testLabels, text_testQueries, audio_testQueries = loadData(FLAGS)
	

	## Update FLAG parameters
	FLAGS.class_size = trainLabels.shape[1]
	FLAGS.input_dims = 200
	FLAGS.input_dims_audio= audio_trainQueries.shape[1]
	FLAGS.input_dims_text = text_trainQueries.shape[1]
	...
	with tf.Graph().as_default():
		tf.compat.v1.set_random_seed(1234) # Graph level random seed

		sess = tf.compat.v1.Session(config=session_conf) # Defining the session of the Graph
		with sess.as_default():
			...
			for t in range(1, FLAGS.epochs+1):
				...
				for start, end in batches:
					...
					query_audio = audio_trainQueries[start:end]
					query_text = text_trainQueries[start:end]
					...
					if answers.shape[0] < FLAGS.batch_size:
						...
						query_audio = np.concatenate( (query_audio, np.zeros( (FLAGS.batch_size-query_audio.shape[0],query_audio.shape[1]) , dtype=np.float32))  , axis=0)
						query_text = np.concatenate( (query_text, np.zeros( (FLAGS.batch_size-query_text.shape[0],query_text.shape[1]) , dtype=np.float32))  , axis=0)
						answers = np.concatenate( (answers, np.zeros( (FLAGS.batch_size-answers.shape[0],answers.shape[1]) , dtype=np.float32))  , axis=0)

					cost_t = model.batch_fit(histOwn, histOther, histOwnMask, histOtherMask, mask, query_audio, query_text, answers, lr, FLAGS.dropout_keep_prob, training_mode=True)
					total_cost += cost_t
				...
				if t % FLAGS.evaluation_interval == 0:
					...
					for start, end in evalTrainBatches:
						...
						if answers.shape[0] < FLAGS.batch_size:
							...
							query_text = np.concatenate( (query_text, np.zeros( (FLAGS.batch_size-query_text.shape[0],query_text.shape[1]) , dtype=np.float32))  , axis=0)
							answers = np.concatenate( (answers, np.zeros( (FLAGS.batch_size-answers.shape[0],answers.shape[1]) , dtype=np.float32))  , axis=0)
						loss, pred= model.predict(histOwn, histOther, histOwnMask, histOtherMask, mask, query_audio, query_text,  FLAGS.dropout_keep_prob, answers, training_mode=False)
					...
					for start, end in val_batches:
						...
						query_audio = audio_valQueries[start:end]
						query_text = text_valQueries[start:end]
						...
						if histOwn.shape[0] < FLAGS.batch_size:
							...
							query_text = np.concatenate( (query_text, np.zeros( (FLAGS.batch_size-query_text.shape[0],query_text.shape[1]) , dtype=np.float32))  , axis=0)
							answers = np.concatenate( (answers, np.zeros( (FLAGS.batch_size-answers.shape[0],answers.shape[1]) , dtype=np.float32))  , axis=0)
						loss, pred = model.predict(histOwn, histOther, histOwnMask, histOtherMask, mask, query_audio, query_text, 1.0, answers, training_mode=False)
						val_preds += list(pred)
						val_loss += loss
					...
					for start, end in test_batches:
						...
						query_audio = audio_testQueries[start:end]
						query_text = text_testQueries[start:end]
