...
class PadDP():
    def process(self, data, col_in, col_out, max_length, axis=0):        
        def fn(x):
...
class ToNumpyDP():
    def process(self, data, col_in):
        return np.stack(data[col_in])

class SqueezeDP():
    def process(self, data, col_in, col_out):
        data[col_out] = data[col_in].apply(lambda x: np.squeeze(x))
        return data

class LoadDataframeDP():
    def process(self, col_in, col_out, data, exclude_cols=None, include_cols=None, order_by=None ):        
        print(type(data[col_in]))
        def fn(x):
            if type(x).__name__ == 'GenericFile':
                x = x.load()
...
class DownsampleDP():
    def process(self, data, factor, col_in, col_out, axis =0, mode='mean'):
...
class NormalizeDP():
    def process(self, data, column_in, column_out, statistics, columns = None):
        ...
        if len(statistics.keys()) > 0:
            for column_in, column_out in zip(column_in,column_out):
                col_stats = statistics[column_in]
                ...
                if group == 'global':
                    ...
                    data[column_out] = data.apply(lambda x: (x[column_in] - g_stats['mean'])/g_stats['std'],axis=1)
                else:
                    ...
                    data[column_out] = data.apply(lambda x: (x[column_in] - col_stats[group][x[group]]['mean'])/(col_stats[group][x[group]]['std']),axis=1)
        return data
