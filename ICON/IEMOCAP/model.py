    ...
    def _build_vars(self):
        ...
        with tf.compat.v1.variable_scope(self._name):
            ...
            with tf.compat.v1.variable_scope("globalGRU"):
                self.globalGRU = tf.compat.v1.nn.rnn_cell.GRUCell(num_units=self._embedding_size, reuse = tf.compat.v1.AUTO_REUSE, name='globalGRU')

            with tf.compat.v1.variable_scope("memoryGRU"):
                self.memoryGRU = tf.compat.v1.nn.rnn_cell.GRUCell(num_units=self._embedding_size, reuse = tf.compat.v1.AUTO_REUSE, name='memoryGRU')
    ...
    def _inference(self, histories_own, histories_other, queries):
        ...
        with tf.compat.v1.variable_scope(self._name):
            ...
            with tf.compat.v1.variable_scope("localGRU"):
                ...
                otherHistoryRNNOutput = tf.nn.dropout(otherHistoryRNNOutput, rate = 1 - (self._dropout), name = "other_rnn_dropout")
            ...
            with tf.compat.v1.variable_scope("globalGRU"):                 
                ...
                for hop in range(self._hops):
                    ...
                    rnn_outputs, final_state = tf.compat.v1.nn.dynamic_rnn(rnn_cell, rnn_input, dtype=tf.float32)
